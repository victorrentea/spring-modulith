...
//  @Query(value = """
//      SELECT p FROM Product p
//      JOIN INVENTORY.STOCK stock ON p.id = stock.productId
//      WHERE UPPER(p.name) LIKE UPPER(?1)
//      AND stock.stock > 0""",nativeQuery = true)
//  List<Product> searchJoinViewHACK_DONT(String namePart, PageRequest pageRequest);
}
